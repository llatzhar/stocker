...
GitHub Actions で毎朝自動実行し、ファンドの基準価額の最新情報を Discord に通知するツール。
...
.github/
  workflows/
    daily_notify.yml  # GitHub Actions ワークフロー定義
```
...
### ワークフロー（`.github/workflows/daily_notify.yml`）

- **スケジュール:** 毎朝 9:00 JST（UTC 00:00）に自動実行
- **手動実行:** GitHub UI の "Run workflow" ボタンでも実行可能
